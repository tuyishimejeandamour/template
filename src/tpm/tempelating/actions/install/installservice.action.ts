...
// export const unpackAndInstall = (tempPath:string):Promise<any>=>{
//     console.log(tempPath);
//   return Promise.resolve({file:'123',name:'hello'});
// }
...
export function collectAllTemplateFiles(cwd: string, useYarn?: boolean): Promise<string[]> {
...
export function getTemplateFiles(
  ...
  useYarn?:boolean
	...
	return collectAllTemplateFiles(cwd,useYarn).then(files => {
...
export async function unpackAndInstall(temppath:string): Promise<IPackageResult> {
	const cwd = temppath;

	const composition = await readcomposition(cwd);

	const files = await gatherFileFromCompressed(composition,cwd);
	...
	const packagePath = await getPackagePath(cwd, composition);
	await compressTemplate(files, path.resolve(packagePath));

	return { composition, packagePath, files };
}
export function gatherFileFromCompressed(composition: IPackageTemplate,temppath:string): Promise<IFile[]> {
	const cwd =  temppath;
	const processors = createDefaultDeProcessors(composition);
